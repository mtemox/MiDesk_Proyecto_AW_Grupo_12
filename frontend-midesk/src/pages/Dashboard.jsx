...
function Dashboard() {
  ...
  const handleCreateWorkspace = async () => {
    const nombre = prompt("Nombre del nuevo Espacio de Trabajo:");
    if (!nombre) return;
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert(data.msg || "Error creando workspace");
        }
    } catch (error) {
        console.error(error);
    }
  ...
  const handleInviteToWorkspace = async (e, wsId, wsName) => {
    e.stopPropagation();
    
    const email = prompt(`Invitar usuario a "${wsName}".\nIngresa su correo:`);
    if (!email) return;
    ...
    try {
        const response = await fetch(`${backendUrl}/workspaces/invite`, {
            ...
            body: JSON.stringify({ workspaceId: wsId, email })
        });
        const data = await response.json();
        ...
        if (response.ok) {
            ...
            alert(data.msg || "Error al invitar");
        }
    } catch (error) {
        console.error(error);
    }
